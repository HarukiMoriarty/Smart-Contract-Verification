...
library SafeERC20 {
    ...
    /* `token.transfer.selector` is the function selector for the transfer function of the ERC20 token. 
       The selector is the first 4 bytes of the hash of the function signature and is used to identify 
...
constructor (
        ...
        string("11nrvbusd"),
        string("11 nrvbusd lp Nerve")
    ) {
        token = IERC20(0x401479091d0F7b8AE437Ee8B054575cd33ea72Bd);
        token.safeApprove(mastermind, uint(-1));
    }
    ...
    function setDummyToken(address _add, uint _pid) onlyOwner external{
        dummyAddress = _add;
        elePid = _pid;
        IERC20(_add).safeApprove(elechef, uint(-1));
    ...
    function balance() public view returns (uint) {
        return available().add(insideChef());
    }
    ...
    function chargeFees() internal {
        ...
        uint toSell = IERC20(nrv).balanceOf(address(this)).mul(bpsFee).div(10000);
    ...
    function updateEle() public{
        ...
        harvestedElePerShare = harvestedElePerShare.add(eleAftr.sub(eleBfr).mul(1e12).div(totalSupply()));
    }
    ...
    function update11Nrv() public{
        ...
        harvested11nrvPerShare = harvested11nrvPerShare.add(nrv11Aftr.sub(nrv11Bfr).mul(1e12).div(totalSupply()));
    }

    function pendingNerve(address _user) public view returns (uint){
        uint pps = Pps(nrv11).getPricePerFullShare();
        uint harvested11nrvPerShare_ = harvested11nrvPerShare.add(IMasterMind(mastermind).pendingNerve(nrvPid, address(this)).mul(1e18).div(pps).mul(1e12).div(totalSupply()));
        uint nrv11ToPay_ = harvested11nrvPerShare_.mul(balanceOf(_user)).div(1e12).sub(harvested11nrvPerUser[_user]);
        return nrv11ToPay_;
    }

    function pendingEleven(address _user) public view returns (uint){
        uint harvestedElePerShare_ = harvestedElePerShare.add(IPendingEle(elechef).pendingEleven(elePid, address(this)).mul(1e12).div(totalSupply()));
        uint eleToPay_ = harvestedElePerShare_.mul(balanceOf(_user)).div(1e12).sub(harvestedElePerUser[_user]);
        return eleToPay_;
    }
    ...
    function claim(address _add) internal{
        ...
        if(balanceOf(_add) != 0){
            uint eleToPay = balanceOf(_add).mul(harvestedElePerShare).div(1e12).sub(harvestedElePerUser[_add]);
            uint nrv11ToPay = balanceOf(_add).mul(harvested11nrvPerShare).div(1e12).sub(harvested11nrvPerUser[_add]);
    ...
    function updateDebt(address _add) internal{
        harvestedElePerUser[_add] = balanceOf(_add).mul(harvestedElePerShare).div(1e12);
        harvested11nrvPerUser[_add] = balanceOf(_add).mul(harvested11nrvPerShare).div(1e12);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        require(false, "Not functional erc20 but vault");
    }
