...
contract Token is ERC20, Ownable, WhiteList {
    ...
    function _transfer(
        ...
        if (
            parentAddress[recipient] == address(0) && recipient != uniswapV2Pair
        ) {
        ...
        if (!isWhiteListed(sender) && !isWhiteListed(recipient)) {
            ...
            if (recipient == uniswapV2Pair) {
                require(
                    senderBalance >= amount.add(fee),
                    "ERC20: There are not enough charges for the account balance"
                );
                unchecked {
